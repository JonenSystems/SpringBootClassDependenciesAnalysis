specification_id: SPC-001.007-001
specification_title: 引数型依存の解析仕様
## spec:
- [前提] SpringBootアプリケーションのソースコードが`targetProjectPath`配下に存在し、H2インメモリデータベースとJPAプロファイル`jpa`が初期化済であること。
- [入力] 解析オペレータが`targetProjectPath`と`targetPackagePattern`を指定し、`.java`（メソッド宣言）を対象に引数型依存を抽出する。
- [処理] JavaParserを用いて`MethodDeclaration.getParameters()`の型を抽出。抽出結果を依存レコードとして整形し、`class_dependencies`テーブルへ登録する。
- [出力] H2上の`class_dependencies`テーブルに引数型依存（呼び出し側に要求する引数型の依存）のレコードを記録し、解析結果画面にパッケージごとのサマリを反映する。
...
|解析対象バッチ識別子|analysisBatchId|String|本仕様用の依存種別（例:`001_007`）と紐付く実行ID。|
...
|依存先識別子|targetIdentifier|String|class_dependencies|引数型依存（呼び出し側に要求する引数型の依存）で参照する対象の完全修飾名。|
|依存種類コード|dependencyKindCode|String|class_dependencies|本仕様に対応する依存種別コード（例:`001_007`）。|
...
flowchart TD
    ...
    proc_req_001_007((プロセス 引数型依存解析))
    analyzer -- 解析パラメータ --> proc_req_001_007
    sourceRepo -- .java（メソッド宣言） --> proc_req_001_007
    proc_req_001_007 -- 引数型依存依存レコード --> dependencyStore
