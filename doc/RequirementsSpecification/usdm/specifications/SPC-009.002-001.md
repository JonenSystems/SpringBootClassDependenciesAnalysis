specification_id: SPC-009.002-001
specification_title: Service→Repositoryの解析仕様
## spec:
- [前提] SpringBootアプリケーションのソースコードが`targetProjectPath`配下に存在し、H2インメモリデータベースとJPAプロファイル`jpa`が初期化済であること。
- [入力] 解析オペレータが`targetProjectPath`と`targetPackagePattern`を指定し、`.java`（DI/呼出）を対象にService→Repositoryを抽出する。
- [処理] JavaParserを用いてServiceでRepositoryのDI/メソッド呼び出しを抽出。抽出結果を依存レコードとして整形し、`class_dependencies`テーブルへ登録する。
- [出力] H2上の`class_dependencies`テーブルにService→Repository（層間の正しい依存）のレコードを記録し、解析結果画面にパッケージごとのサマリを反映する。
...
|解析対象バッチ識別子|analysisBatchId|String|本仕様用の依存種別（例:`009_002`）と紐付く実行ID。|
...
|依存先識別子|targetIdentifier|String|class_dependencies|Service→Repository（層間の正しい依存）で参照する対象の完全修飾名。|
|依存種類コード|dependencyKindCode|String|class_dependencies|本仕様に対応する依存種別コード（例:`009_002`）。|
...
flowchart TD
    ...
    proc_req_009_002((プロセス Service→Repository解析))
    analyzer -- 解析パラメータ --> proc_req_009_002
    sourceRepo -- .java（DI/呼出） --> proc_req_009_002
    proc_req_009_002 -- Service→Repository依存レコード --> dependencyStore
...
flowchart TD
    ...
    collect[Service→Repository対象ソースを走査]
    hasSource{解析対象は存在するか?}
    parse[JavaParserでASTを生成]
    extract[Service→Repositoryを抽出]
