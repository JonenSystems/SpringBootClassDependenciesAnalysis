specification_id: SPC-002.002-001
specification_title: @Bean提供の解析仕様
## spec:
- [前提] SpringBootアプリケーションのソースコードが`targetProjectPath`配下に存在し、H2インメモリデータベースとJPAプロファイル`jpa`が初期化済であること。
- [入力] 解析オペレータが`targetProjectPath`と`targetPackagePattern`を指定し、`.java`（@Beanメソッド）を対象に@Bean提供を抽出する。
- [処理] JavaParserを用いて戻り値型＝Bean型として依存登録。抽出結果を依存レコードとして整形し、`class_dependencies`テーブルへ登録する。
- [出力] H2上の`class_dependencies`テーブルに@Bean提供（`@Configuration`内の`@Bean`でBeanを生成する依存）のレコードを記録し、解析結果画面にパッケージごとのサマリを反映する。
...
|解析対象バッチ識別子|analysisBatchId|String|本仕様用の依存種別（例:`002_002`）と紐付く実行ID。|
...
|依存先識別子|targetIdentifier|String|class_dependencies|@Bean提供（`@Configuration`内の`@Bean`でBeanを生成する依存）で参照する対象の完全修飾名。|
|依存種類コード|dependencyKindCode|String|class_dependencies|本仕様に対応する依存種別コード（例:`002_002`）。|
...
flowchart TD
    ...
    proc_req_002_002((プロセス @Bean提供解析))
    analyzer -- 解析パラメータ --> proc_req_002_002
    sourceRepo -- .java（@Beanメソッド） --> proc_req_002_002
    proc_req_002_002 -- @Bean提供依存レコード --> dependencyStore
