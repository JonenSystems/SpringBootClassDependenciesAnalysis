specification_id: SPC-003.002-001
specification_title: JPAエンティティの解析仕様
## spec:
- [前提] SpringBootアプリケーションのソースコードが`targetProjectPath`配下に存在し、H2インメモリデータベースとJPAプロファイル`jpa`が初期化済であること。
- [入力] 解析オペレータが`targetProjectPath`と`targetPackagePattern`を指定し、`.java`（クラス注釈/フィールド）を対象にJPAエンティティを抽出する。
- [処理] JavaParserを用いて`@Entity`/`@Id`/`@Column`を抽出してスキーマ依存を記録。抽出結果を依存レコードとして整形し、`class_dependencies`テーブルへ登録する。
- [出力] H2上の`class_dependencies`テーブルにJPAエンティティ（`@Entity`でDBテーブルとマッピング）のレコードを記録し、解析結果画面にパッケージごとのサマリを反映する。
...
|解析対象バッチ識別子|analysisBatchId|String|本仕様用の依存種別（例:`003_002`）と紐付く実行ID。|
...
|依存先識別子|targetIdentifier|String|class_dependencies|JPAエンティティ（`@Entity`でDBテーブルとマッピング）で参照する対象の完全修飾名。|
|依存種類コード|dependencyKindCode|String|class_dependencies|本仕様に対応する依存種別コード（例:`003_002`）。|
...
flowchart TD
    ...
    proc_req_003_002((プロセス JPAエンティティ解析))
    analyzer -- 解析パラメータ --> proc_req_003_002
    sourceRepo -- .java（クラス注釈/フィールド） --> proc_req_003_002
    proc_req_003_002 -- JPAエンティティ依存レコード --> dependencyStore
...
flowchart TD
    ...
    collect[JPAエンティティ対象ソースを走査]
    hasSource{解析対象は存在するか?}
    parse[JavaParserでASTを生成]
    extract[JPAエンティティを抽出]
