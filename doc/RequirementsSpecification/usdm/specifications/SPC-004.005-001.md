specification_id: SPC-004.005-001
specification_title: ビルド依存の解析仕様
## spec:
- [前提] SpringBootアプリケーションのソースコードが`targetProjectPath`配下に存在し、H2インメモリデータベースとJPAプロファイル`jpa`が初期化済であること。
- [入力] 解析オペレータが`targetProjectPath`と`targetPackagePattern`を指定し、`pom.xml`/`build.gradle`を対象にMaven/Gradleを抽出する。
- [処理] Maven / Gradle依存解決を用いて依存ツリーを出力（`mvn dependency:tree`）して実行クラスパスを確定。抽出結果を依存レコードとして整形し、`class_dependencies`テーブルへ登録する。
- [出力] H2上の`class_dependencies`テーブルにMaven/Gradle（依存範囲（scope）や伝播依存の影響）のレコードを記録し、解析結果画面にパッケージごとのサマリを反映する。
...
|解析対象バッチ識別子|analysisBatchId|String|本仕様用の依存種別（例:`004_005`）と紐付く実行ID。|
...
|依存先識別子|targetIdentifier|String|class_dependencies|Maven/Gradle（依存範囲（scope）や伝播依存の影響）で参照する対象の完全修飾名。|
|依存種類コード|dependencyKindCode|String|class_dependencies|本仕様に対応する依存種別コード（例:`004_005`）。|
...
flowchart TD
    ...
    proc_req_004_005((プロセス Maven/Gradle解析))
    analyzer -- 解析パラメータ --> proc_req_004_005
    sourceRepo -- pom.xml/build.gradle --> proc_req_004_005
    proc_req_004_005 -- Maven/Gradle依存レコード --> dependencyStore
...
flowchart TD
    ...
    collect[Maven/Gradle対象ソースを走査]
    hasSource{解析対象は存在するか?}
    parse[JavaParserでASTを生成]
    extract[Maven/Gradleを抽出]
