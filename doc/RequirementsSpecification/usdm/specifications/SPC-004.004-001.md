specification_id: SPC-004.004-001
specification_title: オートコンフィグの解析仕様
## spec:
- [前提] SpringBootアプリケーションのソースコードが`targetProjectPath`配下に存在し、H2インメモリデータベースとJPAプロファイル`jpa`が初期化済であること。
- [入力] 解析オペレータが`targetProjectPath`と`targetPackagePattern`を指定し、`pom.xml`/`META-INF/*`を対象にオートコンフィグを抽出する。
- [処理] Maven依存解析 / jdepsを用いてPOMの`spring-boot-starter-*`から推定、`spring.factories`/`AutoConfiguration`を一覧化。抽出結果を依存レコードとして整形し、`class_dependencies`テーブルへ登録する。
- [出力] H2上の`class_dependencies`テーブルにオートコンフィグ（Starter導入で自動登録されるBean依存）のレコードを記録し、解析結果画面にパッケージごとのサマリを反映する。
...
|解析対象バッチ識別子|analysisBatchId|String|本仕様用の依存種別（例:`004_004`）と紐付く実行ID。|
...
|依存先識別子|targetIdentifier|String|class_dependencies|オートコンフィグ（Starter導入で自動登録されるBean依存）で参照する対象の完全修飾名。|
|依存種類コード|dependencyKindCode|String|class_dependencies|本仕様に対応する依存種別コード（例:`004_004`）。|
...
flowchart TD
    ...
    proc_req_004_004((プロセス オートコンフィグ解析))
    analyzer -- 解析パラメータ --> proc_req_004_004
    sourceRepo -- pom.xml/META-INF/* --> proc_req_004_004
    proc_req_004_004 -- オートコンフィグ依存レコード --> dependencyStore
