specification_id: SPC-004.002-001
specification_title: 構成プロパティの解析仕様
## spec:
- [前提] SpringBootアプリケーションのソースコードが`targetProjectPath`配下に存在し、H2インメモリデータベースとJPAプロファイル`jpa`が初期化済であること。
- [入力] 解析オペレータが`targetProjectPath`と`targetPackagePattern`を指定し、`.java`（@ConfigurationProperties）/YAMLを対象に構成プロパティを抽出する。
- [処理] JavaParser / YAMLパーサを用いてprefix配下のキー群→プロパティクラスのフィールド構造にマップ。抽出結果を依存レコードとして整形し、`class_dependencies`テーブルへ登録する。
- [出力] H2上の`class_dependencies`テーブルに構成プロパティ（`@ConfigurationProperties`の型安全バインド依存）のレコードを記録し、解析結果画面にパッケージごとのサマリを反映する。
...
|解析対象バッチ識別子|analysisBatchId|String|本仕様用の依存種別（例:`004_002`）と紐付く実行ID。|
...
|依存先識別子|targetIdentifier|String|class_dependencies|構成プロパティ（`@ConfigurationProperties`の型安全バインド依存）で参照する対象の完全修飾名。|
|依存種類コード|dependencyKindCode|String|class_dependencies|本仕様に対応する依存種別コード（例:`004_002`）。|
...
flowchart TD
    ...
    proc_req_004_002((プロセス 構成プロパティ解析))
    analyzer -- 解析パラメータ --> proc_req_004_002
    sourceRepo -- .java（@ConfigurationPropert窶ｦ --> proc_req_004_002
    proc_req_004_002 -- 構成プロパティ依存レコード --> dependencyStore
