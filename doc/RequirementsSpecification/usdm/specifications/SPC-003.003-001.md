specification_id: SPC-003.003-001
specification_title: クエリメソッドの解析仕様
## spec:
- [前提] SpringBootアプリケーションのソースコードが`targetProjectPath`配下に存在し、H2インメモリデータベースとJPAプロファイル`jpa`が初期化済であること。
- [入力] 解析オペレータが`targetProjectPath`と`targetPackagePattern`を指定し、`.java`（Repositoryインタフェース）を対象にクエリメソッドを抽出する。
- [処理] JavaParserを用いて`MethodDeclaration`名規約/`@Query`文字列を収集（SQL/JPQL参照も注記）。抽出結果を依存レコードとして整形し、`class_dependencies`テーブルへ登録する。
- [出力] H2上の`class_dependencies`テーブルにクエリメソッド（メソッド名規約や`@Query`による依存）のレコードを記録し、解析結果画面にパッケージごとのサマリを反映する。
...
|解析対象バッチ識別子|analysisBatchId|String|本仕様用の依存種別（例:`003_003`）と紐付く実行ID。|
...
|依存先識別子|targetIdentifier|String|class_dependencies|クエリメソッド（メソッド名規約や`@Query`による依存）で参照する対象の完全修飾名。|
|依存種類コード|dependencyKindCode|String|class_dependencies|本仕様に対応する依存種別コード（例:`003_003`）。|
...
flowchart TD
    ...
    proc_req_003_003((プロセス クエリメソッド解析))
    analyzer -- 解析パラメータ --> proc_req_003_003
    sourceRepo -- .java（Repositoryインタフェース） --> proc_req_003_003
    proc_req_003_003 -- クエリメソッド依存レコード --> dependencyStore
