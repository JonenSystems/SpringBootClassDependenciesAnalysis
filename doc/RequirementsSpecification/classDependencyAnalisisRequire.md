...
### クラス名の重複チェック機能
解析結果の表示時に、同一パッケージ内で同じクラス名が複数回表示されることを防ぐため、クラス名の重複チェック機能を実装する。

#### 機能概要
- 解析結果のサマリ作成時に、クラス名リストから重複を除去する
- 重複除去後、クラス名をアルファベット順にソートして表示する
- クラス数のカウントも重複除去後の数で表示する

#### 実装方法
- Javaの`Stream.distinct()`メソッドを使用してクラス名の重複を除去
- `Set<Class>`を使用してエンティティレベルでの重複も防止
- `Class`エンティティの`equals`と`hashCode`は`fullQualifiedName`に基づいて実装
